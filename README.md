...
1. Only fields can be decorated with `ProtoMember`. .NET **nanoFramework** lacks `Type.GetProperties` at the moment. All code to support properties is present in the codebase, so this can be easily added when `GetProperties` is implemented. However, you can decorate with `ProtoMember` the `set` and `get` methods for properties. This allows you to work with properties in the same way as with fields.
